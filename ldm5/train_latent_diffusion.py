"""Stage 2: Pretrain the latent-space DDPM U-Net.

The pretrained `AutoencoderKL` is frozen. Five-channel seismic tensors are
encoded to latents, noised by a `DDPMScheduler`, and a `UNet2DModel` is
trained to predict the noise.

Usage:
    python -m ldm5.train_latent_diffusion --ae_dir outputs/autoencoder/final
"""
...
from .models import build_scheduler, build_unet
...
def parse_args():
    ...
    p.add_argument("--ae_dir", type=str, required=True, help="Pretrained AutoencoderKL directory.")
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir) / "latent_diffusion"
    ...
    # Sanity-check latent shape vs UNet sample size.
    with torch.no_grad():
    ...
    unet = build_unet(cfg.unet).to(args.device)
    ...
    for epoch in range(args.num_epochs):
        unet.train()
        pbar = tqdm(loader, desc=f"LDM epoch {epoch + 1}/{args.num_epochs}")
        for step, batch in enumerate(pbar):
            ...
            if args.smoke:
                logger.info("Smoke test OK: one latent-DDPM step ran successfully.")
                logger.close()
                return
        ...
        if (epoch + 1) % args.save_every == 0 or (epoch + 1) == args.num_epochs:
            ...
            logger.info(f"Saved U-Net checkpoint to {ckpt_dir}")

    unet.save_pretrained(out_dir / "final")
    logger.info(f"Final U-Net saved to {out_dir / 'final'}")
    logger.close()

...
from .losses import horizon_loss, layer_sync_loss
...
def parse_args():
    ...
    p.add_argument("--skip_ldm", action="store_true", help="Only evaluate AE reconstruction.")
    return p.parse_args()
...
def evaluate_ae(ae, dataset, channels, mean, std, args):
    ...
    for i, batch in enumerate(tqdm(loader, desc="AE eval")):
        ...
        horizon_sum += horizon_loss(recon, x).detach().cpu().item()
        sync_sum += layer_sync_loss(recon, x).detach().cpu().item()
    ...
    report["horizon_l1_mean"] = horizon_sum / max(n_batches, 1)
    report["layer_sync_l1_mean"] = sync_sum / max(n_batches, 1)
    return report
...
def main():
    ...
    print(f"  horizon L1: {ae_report['horizon_l1_mean']:.4f}")
    print(f"  sync L1   : {ae_report['layer_sync_l1_mean']:.4f}")

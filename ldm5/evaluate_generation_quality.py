...
Usage:
    python -m ldm5.evaluate_generation_quality \
        --input_npz outputs/evaluation/visualize_arrays.npz
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input_npz",
        type=str,
        default="outputs/evaluation/visualize_arrays.npz",
        help="NPZ containing real/generated arrays with shape [N,C,H,W].",
    )
    parser.add_argument(
        "--save_root",
        type=str,
        default="outputs/generation_quality",
        help="Root directory for generation-quality evaluation runs.",
    )
    parser.add_argument(
        "--run_name",
        type=str,
        default=None,
        help="Optional run folder name under --save_root/<experiment>. Defaults to a timestamp.",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default=None,
        help="Exact output directory. Overrides --save_root and --run_name.",
    )
...
def default_save_dir(input_path: Path, save_root: str | Path, run_name: str | None) -> Path:
    experiment_name = input_path.parent.name or input_path.stem
    if experiment_name in {".", ""}:
        experiment_name = input_path.stem
    run_id = run_name or datetime.now().strftime("%Y%m%d_%H%M%S")
    return Path(save_root) / experiment_name / run_id
...
def main() -> None:
    ...
    if args.save_dir:
        ...
        save_dir = default_save_dir(input_path, args.save_root, args.run_name)
    ...
    metadata = {
        ...
        "edge_top_quantile": args.edge_top_quantile,
        "blur_peak_fraction": args.blur_peak_fraction,
    }
